...
async def vk_webhook(request: Request):
    ...
    COMMANDS = {
        "начать","статистика","сменить предмет","сменить экзамен",
        "базовый","средний","сложный","теория","тест","практика"
    }
    ...
    cur.execute("""
        SELECT exam, subject, question, waiting_for_answer,
               difficulty, task_type, solved_count
        FROM user_progress WHERE vk_user_id=%s
    """, (user_id,))
    row = cur.fetchone()

    # ===== INIT =====
    if not row:
        cur.execute("INSERT INTO user_progress (vk_user_id) VALUES (%s)", (user_id,))
        conn.commit()
        vk_send(user_id, "Привет! Нажмите «Начать».", get_main_keyboard())
        conn.close()
        return PlainTextResponse("ok")

    exam, subject, question, waiting, difficulty, task_type, solved = row

    # ===== СТАТИСТИКА =====
    if text_lower == "статистика":
        vk_send(user_id, f"📊 Решено вопросов: {solved or 0}", get_game_keyboard())
        conn.close()
        return PlainTextResponse("ok")

    # ===== СМЕНА =====
    if text_lower == "сменить экзамен":
        cur.execute("UPDATE user_progress SET exam=NULL, subject=NULL WHERE vk_user_id=%s",(user_id,))
        conn.commit()
        vk_send(user_id,"Выберите экзамен:",get_exam_keyboard())
        conn.close()
        return PlainTextResponse("ok")

    if text_lower == "сменить предмет" and exam:
    ...
    # ===== ОТВЕТ =====
    if waiting and text_lower not in COMMANDS:
        ...
        q = generate_question(exam, subject, difficulty, task_type)
        cur.execute("""
            UPDATE user_progress
    ...
    if text_upper in ("ОГЭ","ЕГЭ"):
        cur.execute("UPDATE user_progress SET exam=%s WHERE vk_user_id=%s",(text_upper,user_id))
        conn.commit()
    ...
    if exam and not subject:
        ...
        conn.commit()
        vk_send(user_id,"Выберите сложность:",get_difficulty_keyboard())
        conn.close()
        return PlainTextResponse("ok")

    if text_lower in ("базовый","средний","сложный"):
        cur.execute("UPDATE user_progress SET difficulty=%s WHERE vk_user_id=%s",(text_lower,user_id))
        conn.commit()
        vk_send(user_id,"Выберите тип задания:",get_task_type_keyboard())
        conn.close()
        return PlainTextResponse("ok")

    if text_lower in ("теория","тест","практика"):
        cur.execute("UPDATE user_progress SET task_type=%s WHERE vk_user_id=%s",(text_lower,user_id))
        conn.commit()
        vk_send(user_id,"Нажмите «Начать»",get_game_keyboard())
        conn.close()
        return PlainTextResponse("ok")

    if text_lower == "начать" and exam and subject:
        q = generate_question(exam, subject, difficulty, task_type)
        cur.execute("""
            UPDATE user_progress SET question=%s, waiting_for_answer=true
            WHERE vk_user_id=%s
        """,(q,user_id))
        conn.commit()
        vk_send(user_id,"Вопрос:\n"+q,get_game_keyboard())
        conn.close()
        return PlainTextResponse("ok")

    vk_send(user_id,"Используйте кнопки.",get_game_keyboard())
    conn.close()
    return PlainTextResponse("ok")
